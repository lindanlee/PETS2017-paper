...
and interviews (Section~\ref{sec:qualitative}). This feedback was used to make changes to the interface (Section~\ref{redesign}). The second user study is a large-scale, many-to-one experiment
...
The options under ``Transport type'' denote various
...
\begin{enumerate}
    ...
    \item {\bfseries  Time to completion:} How long it takes between program startup and a successful Tor bootstrap, which includes time spent searching online for help and waiting for a the connection to bootstrap.
...
Some dutifully followed the directions to wait, while others are lost for what to do and choose to wait in frustration. Either way, we think that people spent too much time waiting. 
\end{description} 

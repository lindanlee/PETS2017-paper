...
\subsection{Results} 
In this section, we discuss problems encountered by our participants at each step of the configuration process and solutions to those problems. We conclude with our goals for improving the configuration interface. Because of the small sample size of qualitative studies, we can only state that these are problems that participants can encounter when configuring. All quotes are paraphrased from live transcriptions of participant interviews.\\

%We noticed four common challenges:  
%
%\begin{itemize}
%\item {\bfseries Challenge 1:} People are not qualified to answer the bridge and proxy questions asked or to choose between the transport choices given.  
%\item {\bfseries Challenge 2:} People feel compelled to configure a connection, even if they don't need one. 
%\item {\bfseries Challenge 3:} It was common for users to configure proxies, rather than trouble shooting bridges. 
%\item {\bfseries Challenge 4:} When the first attempt to connect fails, people did not know what to do or what to try next. The feedback did not guide users into specific actions.
%\end{itemize}
...
We also made additional changes to unburden the user (5-8) and build a mental model (9-10):\\
...
\item {\bfseries Got rid of proxy and bridge questions.}  We removed the bridge and proxy questions, which were highly technical and challenging for users to answer. There are two fewer screens in the interface.
...
\item {\bfseries Added explicit advice on choosing bridges.} The default bridge is still obfs3. There is added text that advises users to try a meek bridge if obfs3 does not work. 
...
We recorded the computer screen throughout each session.
...
\section{Discussion} 
{\color {red} process the results, talking about the speculative here.} 
...
On a fundamental level, there are alternative approaches to manual configuration. An alternative approach has the potential to be highly effective, but would warrant future research: \\

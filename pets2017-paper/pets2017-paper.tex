...
\begin{description}
\item {\bfseries Getting people to click configure if they need it.}  81\% (92 of 114) of first attempts were to connect directly, with 52\% (48 of 92) direct first attempts from the new interface and 47\% (44 of 93) direct first attempts from the old interface.  %giving specific countries
\item {\bfseries Stop users from configuring proxies.} Everyone did this. Participants with the new interface (which explicitly states that they did not need a proxy) spent less configuring proxies comparatively, but we do not understand why users with the new interface spent time configuring proxies after explicitly being told that they do not need one. %redirect to bridge or proxy page depending on error 
\item {\bfseries Help people choose second bridges.}
Participants in E3 were required to choose a particular subset of hard-coded bridges, none of which was the default bridge. After the default bridge, the most chosen bridge was the one listed first in the drop-down menu. Many bridges listed at the end of the drop-down menu were not chosen. We tried to help users by providing instructions on which bridges to choose if the default fails, but it is unclear if this advice helped. 66\% (10 of 15) participants with the new interface and 38\% (5 of 13) participants with the old interface chose meek as their second bridge. %automate after first failure 
...
{\color {blue}
It's not exact censorship, so we cannot make any claims about how difficult it would be for a user to connect to Tor in a certain country or much our improvements will help those people.  

Our study was conducted in a laboratory setting. We believe that our participants were suffiencitnly motivated to complete the task due to the Hawthorne effect (they knew that they were being watched) and the monetary reward. We don't know if this is more or less motivated than people in censorships. We don't have information about when users would give up. 
...
We chose to focus on more common cases, and did not simulate environments that requires users to configure a custom bridge or proxy. But if it's this hard to connec without using the more advanced configuration methods, then out point about needing an alternative method still stands. 

We did not study international users. English on english users. Again, english is the best language though, so if it's this hard for native english speakers in the best tranlated language, it will be harder for others. 
}

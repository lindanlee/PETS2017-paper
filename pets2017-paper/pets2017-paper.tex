...
Tor relays are the routers in the Tor network. The Tor network is a group of routers that peform onion routing, a technique for anonymous communication over a computer network.  
...
through content delivery networks. Fig.~\ref{fig:old-bridge-combobox} shows the bridge and transport options at the time of the study. A user uses a bridge to connect to the Tor network while Tor relays or the Tor network protocol is is being censored. 
...
We performed a cognitive walkthrough~\cite{cognitive-walkthrough} to prepare for user testing. This evaluation method involved usability evaluators to working through a series of tasks from the perspective of the user, trying to understand a system's learnability for new or infrequent users.
...
We opportunistically used our observations to make changes to the interface. We believed that the process of identifying design principles, drafting design changes, and testing an alternative interface would be insightful.
...
and the Windows operating system hosts file to to block websites by
...
\subsection{Procedure}
Three researchers ran the formative usability tests, with each session having at least two researchers.  
...
\subsubsection{E2} 
In E2, a bridge was required to connect. Any of the hardcoded bridges from the dropdown menu, as well as any custom bridges aquired out of band would work. Configuring a proxy was optional.
Observations in E2: 
...
\subsection{Procedure} 
We agreed on security and usability design principles, before reviewing our formative usability study observations and brainstorming changes to the interface. We worked with a user interface designer to iteratively design the interface. Figure~\ref{fig:new-interface} is the result.
...
We confirmed our observations from our formative usability test and find that participants wait the progress screen for a long time, spend time configuring proxies when they did not need to, and do not know what to do if the recommended bridge doesn't work. 
...
Researchers have been studying usable computer security for decades. 
%the paper that started it all: ~\ref{whitten1999johnny}

% methods
qualitative analysis, quantitative analysis, cognitive walkthroughs
%usability inspection: ~\ref{nielsen1994usability}
%heuristic evaluation: ~\ref{nielsen1994heuristic}
%improving dialogue: ~\ref{molich1990improving}

% focuses
graphical redesign, redesigning interactions, user education 

% user help 
% explicit goals and subgoals ~\ref{reeder2005user}
% hypothesis elimination ~\ref{wason1960failure}
% user tolerance on security delays ~\ref{egelmanplease}

% subjects 
Two consistent areas of research are user authentication and email encryption ~\ref{payne2008brief}.
%%% driveby citations of user authentication papers here
%%% driveby citations of email authentication papers here. 
There has not been much usable security work on cencorship circumvention or anonymity.
...
\section{Formative User Study Recruitment Posting} 
...
\section{Formative User Study Prescreening Survey} 
...
\section{Formative User Study Introduction Script} 
...
\section{Formative Study Worksheet Text} 
...
\section{Summative User Study Recruitment Posting}
...
\section{Summative User Study Introduction Script} 
...
\section{Summative User Study Worksheet Text} 
...
\section{Summative User Study Exit Survey} 

...
Tor Browser~\cite{torbrowser} is a modified Firefox browser that routes traffic through the Tor network.
Although Tor and Tor Browser were originally designed for anonymity, many now use them to circumvent Internet censorship.
...
Specifically, we describe the initial evaluation of the interface (Section~\ref{sec:inspection}) and our observations of participants' interactions with it (Section~\ref{sec:qualitative}).
...
Finally, we provide recommendations based on what we learned (Section~\ref{sec:recommendations}), we address limitations of our evaluation (Section~\ref{sec:limitations}), frame our study in the context of existing work (Section~\ref{sec:related}),
...
Qualitative testing is used to gain an understanding of underlying reasons, opinions, and motivations. We use qualitative testing to develop hypotheses about why people struggle to use the interface and devlop ideas for potential design modifications to the interface. 

Quantitative testing is used to quantify the problem by way of generating numerical data or data that can be transformed into useable statistics. We use quantitative testing to quantify how people interact with the interface to uncover user patterns.  
...
We performed our qualitative experiments one participant at a time, so that we could observe their behaviors in real time and interview them about their experience after use.
...
\caption{
A digraph illustrating the potential user paths through the interface. Non-leaf nodes are screens in the user interface (first, bridge~1, bridge~2, bridge help, proxy~1, proxy~2, and progress). Leaf nodes are various end states (success or error). Transitions between these nodes denote user actions. 
}
\label{fig:digraph}
\end{figure*} 
...
\subsection{Procedure} 
We ran the experiments in a room with a monitor, keyboard, and a Windows computer. On a fresh install of the operating system, we installed Tor Browser 5.0.3 for testing, Chrome, Firefox, and Internet Explorer for browser options, and VLC media player~\cite{vlc} for recording the computer screen. Before each participant, we ran a script that simulated E1, E2, or E3 and started recording the computer screen.
...
We kept the user involved in the configuration so that they explicitly start a connection to Tor (currently, the user needs to click a ``connect'' button before Tor Launcher makes a connection to Tor). 
%We chose not to leverage user inputs such as their location, risk, or other preferences, as this would require additional research on all these options before designing changes incorporating those. 
And although some users in our qualitative study tried to configure unnecessary components in confusion, we still give users the freedom to configure optional components. 
We limited our design changes to ones that do not require infrastructural changes or cause additional financial burden to make it easier for the Tor project to adopt them.
...
%Non-finishing participants' configuration time was computed by subtracting the time on the progress screen from their assigned completion time.
}

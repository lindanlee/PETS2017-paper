...
For instance, a user who needs a bridge but not need a proxy can connect to Tor with a bridge and proxy, provided that both were configured correctly. The set of valid configuration settings vary on the censorship environment. 
...
Because proxies are not Tor-specific concept and affects a small fraction of the Internet population, we study connections to Tor that require a bridge but not a proxy. We focus our attention on the parts of the configuration interface that set up bridges and simulate environments that do not need a proxy. However, connections to Tor are still considered valid in the experiment. 

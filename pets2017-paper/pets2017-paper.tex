...
Justify why we kept the similar interface.
...
\subsection{Results} 
124 people participated in our experiment, but we used data from 114. We filtered participants who made errors in completing the consent form, did not understand the task, or did not interact with the instrumented interface. Table~\ref{table:participant-summary} gives an overview of the results, summarizing completion rate, connection time, and configuration time by condition. Figure~\ref{fig:all-participant-edges} summarizes all participants' paths during the experiment.
...
%We believe that the added advice on what to try next on error messages would have improved the rate of success. However, only 1 of 116 participants saw an error during the experiment. 
...
% flashproxy, fte, fte-ipv6, obf4, and scramblesuit were not used! 
...
% connection and configuration time 
...
Although the abovementioned suggestions will improve the deployed interface, we recommend exploring solutions that leverage automation, user-dependent inputs, or network-dependent inputs. We start this discussion by sharing some of our own ideas in appendix~\ref{alternatives}. 

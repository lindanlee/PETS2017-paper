...
\subsection{Results} 
124 people participated in our experiment, but we used data from 113. We filtered participants who made errors in completing the consent form, did not complete the exit survey, did not understand the task, or did not interact with the instrumented interface. In this section, we use OLD to refer to the deployed Tor Browser 5.0.3 Tor Launcher GUI (Figure~\ref{fig:old-interface}) and NEW to refer to our redesigned version (Figure~\ref{fig:new-interface}). Table~\ref{table:participant-summary} gives an overview of the results and Figure~\ref{fig:all-participant-edges} summarizes all participants' paths during the experiment.
...
{\color {red} In total, 63\% (72 of 114) of first attempts to connect failed and 79\% (363 of 458) of total attempts to connect failed.}
...
We only tested the interface on Windows machines, which were the machines available at <redacted>. %Xlab.
...
% Scoping our work wrt usable security, usability, and interaction design  

...
\pquote{P1 (new user, direct, 0:39)}{They made a direct connection immediately.}

%P2(regular): E1 (1:20, 0:22 to 1:42) 
\pquote{P2 (new user, direct, 1:20)}{They spent time reading the text on the start screen, and chose to connect directly because they were intimidated by the other option.}
...
\pquote{P4 (new user, obfs4, 8:56)}{They chose the recommended bridge and then tried to configure a proxy. After not being able to fill out the proxy input fields, make a connection with an obfs4 bridge and no proxy. Note that they didn't try connecting with the obfs3 bridge, which would have worked.}
...
\pquote{P6 (new user, obfs3, 4:04)}{They made a direct connection and giving up after three minutes of waiting. Then, they connected with the recommended bridge.}

%P13 (regular): E2 (7:48; 0:34 to 9:22) 
\pquote{P7 (new user, obfs3, 9:22)}{They made a direct connection. When that did not work, they restarted Tor Launcher and made another direct connection--and repeated this two more times. On the fourth restart, they decided to not use a bridge but tried to configure a proxy, but gave up. On the fifth restart, they connect with the recommended bridge.}

%P12 (regular): E2 (10:40; 0:46 to 11:26)
\pquote{P8 (new user, direct, 10:40)}{They made a direct connection, which fails. They then look at proxy settings, get intimidated, and tried the direct connection again, which fails. They tried configuring a proxy for 4 minutes, but gave up. They tried a direct connection again, which worked. E2 required a bridge to connect, but new public relays came online after we ran our firewall rules and they were able to connect using those relays.}

%P11 (used Tor before): E2 (~3:00; 1:05 to 5:13)
\pquote{P9 (previous Tor user, obfs3, 4:08)}{They made a direct connection. Then, they answered no to both bridge and proxy questions, and effectively made another direct connection. After that, they connect with the recommended bridge.}

%P8 (used Tor): E2 (clock drift mission impossible, 1:38 to 7:20) 
\pquote{P10 (previous Tor user, failed to connect)}{They made a direct connection. Then they made a connection with the recommended bridge (~5:42), which should have worked. However, our setup experienced a clock drift and Tor does not allow connections with bad clocks. The interface did not tell the user about the clock, and they spent the rest of their time trying different bridges and proxies in vain.}
...
{\color {blue}
not trying direct or trying direct, bad progress bar, trying to configure a proxy. How to fix it. 

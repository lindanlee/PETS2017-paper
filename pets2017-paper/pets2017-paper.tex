...
\subsection{Procedure} 
We ran the experiment in a meeting room at UC Berkeley. The room had a desk and chair, where a monitor, keyboard, and a Windows computer were set up. On a clean computer, we downloaded and installed Tor Browser 5.0.3 for testing, Chrome, Firefox, and Internet Explorer so that a participant could browse the Internet in another browser of their choice, and VLC~\cite{vlc} to record the screen during the experiment. Before each participant, we ran a script on the computer that set up one of the three simulated environments (E1, E2, or E3) and started recording the computer screen.

...
rather than an anonymity tool. We evaluate, redesign, and test the interface by
...
Users started using Tor~\cite{dingledine2004tor} as a censorship circumvention tool because Tor's anonymizing functionality also coincidentally circumvented censorship. In fact, censorship circumvention became a common enough use case for Tor that many countries attempt to block Tor for that reason~\cite{winter2012great}. Today, Tor is aware of these users and explicitly provides support for circumventing censorship through a network of unlisted relays as entry points into the Tor network, various methods of obfuscation to make connections to the Tor network indistinguishable, and employs advanced techniques to resist blocking.
...
This paper details two user studies, and a redesign of the Tor configuration interface. 
Both user studies are set in three simulated censorship environments
that required various levels of manual configuration to circumvent.  
Participants were instructed to circumvent censorship and access a blocked website
by configuring Tor Browser and using it to visit the website.
...
% Our experiment was not limited by Xlab requirements.
% [What does that mean?]

...
State level censors block access to the Tor network by blocking Tor relays, which are publicly listed. Some also employ more sophisticated techniques such as deep packet inspection and protocol detection. Any connection to Tor will allow access to censored websites and services. Bridges and transports allow a connection to the Tor network when public relays are blocked and Tor-looking packets are censored. 

A local proxy is required to connect 
in certain managed networks (i.e. company and university firewalls).
...
Tor Browser version~4.5 incorporated textual and navigational changes to Tor Launcher based on our work <redacted Tor blog post here; it was authored by one of the authors of this paper>. The authors have discussed  alternative ways to bootstrap a connection to Tor without requiring users to configure network components with Tor developers, and they were open to considering some of our ideas for future improvements. 

We encourage more researchers to perform this type of usability testing for other parts of Tor Browser and other Tor applications, especially because it provides user insight to company that does not collect user data. You can help by performing usability research that uncovers unknown problems, tests out new ideas, or verifies the impact of recently made changes. We encourage the use of simulated censorship environments as a tool for user testing censorship circumvention software. Not only do simulated environments avoid rerouting traffic through a censoring country's networks, their reproducibility and stability are ideal for experiments. 
...
We asked people how familiar they were with some technical terms and security tools during our prescreening survey for formative usability testing. We did not use these self-reported answers to influence any process of our testing. We show the responses from the particpants that we chose. 

We suspect that participants answered dishonestly in hopes of appearing more technically fluent. We know this because some reported that they were familiar a term that did not exist (x.511 certificates). We inserted this fictional term specifically to detect this. Here are our participants' self-reported familiarity with technical terms: \\

{\color {red} MAKE A TABLE HERE} \\

Participants' self-reported use of security tools. We assume that self-reported use is the upper bound of how many participants have used these security tools. Here are our participants' self-reported use of security tools:\\

...
We concluded with a summative usability test~\cite{summative} that determined the usability of an interface. We test the current interface to evaluate it and test the redesigned interface to see if our design changes had impact. Summative usability testing is a qualtiy assurance tool designed to test features in a design. In short, formative usability tells you what isn't usable, while summative usability testing tells you if it is usable. 20 users is considered to be the minimum number to statistically significant measurments~\cite{howmanyusers}, so we recruited for 20 participants in each experimental condition.  

...
\section{Limitations} 
{\color {red} our study was only run on windows machines. our participants were not motivated like real users would be in real environments to succeed, but also may have been too motivated since they were getting paid. We didn't intentionally test the cases in which the user is required to configure a custom bridge or proxy. Our users were recruited from one geographic location, and their first language is English.} 
...
{\color {red} Rowilma del Castillo for setting up Xlab, Nima Fatemi, Isa (?), Georg (?), and the UX team for giving feedback, and Cecile Basnage for reviewing the UI of circumvention tools.} 

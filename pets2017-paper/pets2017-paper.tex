...
When a participant chooses to configure a bridge, they are faced with a list of options without much explanation. 
\begin{quotation} 
...
If users did not successfully connect on their first attempt, they did not know what to do next. Error messages (see Figure \ref{fig:error}) did not guide users into taking specific actions (such as trying a different bridge, trying the same configuration again, try connecting directly, etc.). Because no information was given, our participants drew their own (incorrect) conclusions after a failed connection. Since the interface passively re-displayed a proxy-related window seen after an attempted connection regardless of the error, many participants were convinced that they needed a proxy to connect to the Tor network.
%proxies
\begin{quotation}
...
\noindent{\bfseries Sources of Frustration} 
%time
\begin{quotation}

...
~\ref{fig:bridges}) to guide users through setting up their connection to the
...
%definition of a bridge, defintion of a proxy here. 
...
\item {\bfseries Path 1}:  Configure directly
\item {\bfseries Path 2}:  Bridge, no proxy
\item {\bfseries Path 3}:  Bridge, proxy 
\item {\bfseries Path 4}:  No bridge, proxy
\item {\bfseries Path 5}:  No bridge, no proxy
\end{itemize}
...
configure a bridge (as seen in TODO, the title of the window during configuration 
...
configure a local proxy (as seen in TODO, the title of the window during
...
\noindent {\bfseries Things that are good} %% fix this so it's not so casual
{\color {red} 
...
\noindent {\bfseries Things that could be improved}%% fix this so it's not so casual
{\color {red} 

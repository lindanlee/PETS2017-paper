...
background, and familiarity with Tor. \\
...
\section{Design}
Based on the feedback we received and the insights we gathered from our
participants, we designed improvements to the launcher interface. This section
discusses our results and the design considerations we employed. \\
...
repercussions for citizens. Furthermore, some users may be turned off by the
...
machine by looking for settings in well-known locations on the filesystem. The
same can be done to answer the question that led many of our participants
astray: is a local proxy even needed? Since this piece of automation offers a
significant potential upside, with no apparent risks, we have incorporated it
into our design. \\
...
connection was censored when, in reality, it was not. It furthermore contributed
...
of stating the underlying options. Separate screens with binary options (Do you
need a proxy? Yes/No) are removed; instead the option to opt out is incorporated
on the configuration screen. This results in a more streamlined interface (fewer
clicks required) and unified the context of each option: all information about
bridges (or proxies) is now contained on one screen, so the user has the full
...
option. A similar aesthetic applies to the connect button seen after custom
configuration.

Rarely used elements are deemphasized. For example, the custom bridges box,
which previously took up a significant portion of the bridge configuration
screen, is now hidden until that option is selected.
...
avoid misleading users into thinking the proxy settings are the culprit. \\
...
prevent such confusion by gracefully handling failures.

The progress bar in the connection window drew many complaints, because it got
stuck and didn't offer any guidance for how long users were supposed to wait. We
address this by providing a countdown timer, indicating how long users should
expect to wait. We further enhance the progress bar by adding visual indicators
...
includes a separate button for retrying without changing settings and another
for returning to reconfigure the application. \\

...
# This is the order to arrange the rows in the output table.
label.order <- c(
	"no bridge, no proxy",
	"obfs3, no proxy",
	"meek-amazon, no proxy",
	"meek-google, no proxy",
	"meek-azure, no proxy",
	"no bridge, 3rd-party proxy"
)
...
if (length(unique(successful.attempts$label)) != length(label.order)) {
	stop(sprintf("something's wrong: we think this table should have %d rows but there are %d", length(label.order), length(unique(successful.attempts$label))))
}
successful.attempts$label <- factor(successful.attempts$label, levels=label.order)

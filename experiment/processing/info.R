...
sink("median_time.tex")
cat("% Do not edit this file. Edit info.R instead.\n")
cat("\\begin{tabular}{l r r r r}\n")
cat("& First & Proxy & Bridge & Progress \\\\\n")
cat("\\noalign{\\hrule}\n")
for (env in levels(screen_time_per_user$env)) {
	for (version in levels(screen_time_per_user$version)) {
		cat(sprintf("%s-%s", env, version))
		...
		for (edges in c("first_edges$duration", "proxy_edges$duration", "bridge_edges$duration", "progress_edges$duration")) {
			cat(sprintf(" & %.f\\%%", 100 * median(env.version[[edges]]/env.version[["edges$duration"]])))
		}
		cat(" \\\\\n")
	}
}
